...
#define CROWN_EXPLOSION_IMG_PATH "assets/projectiles/maga_explosion.gif"
#define CROWN_EXPLOSION_DURABILITY 1
#define CROWN_EXPLOSION_ACTION_COOLDOWN 30
#define CROWN_EXPLOSION_DAMAGE 30
#define CROWN_EXPLOSION_BOUNCE_DECAY 0
#define CROWN_EXPLOSION_COLLISION false
#define CROWN_EXPLOSION_GRAVITY false

void Crown_Explosion_Initialize(Projectile *proj) 
{
    proj->gif               = algif_new_gif(CROWN_EXPLOSION_IMG_PATH, -1);
    proj->width             = proj->gif->width;
    proj->height            = proj->gif->height;
    proj->durability        = CROWN_EXPLOSION_DURABILITY;
    proj->action_cooldown   = 0;
    proj->bounce_decay      = CROWN_EXPLOSION_BOUNCE_DECAY;
    proj->collision         = CROWN_EXPLOSION_COLLISION;
    proj->gravity           = CROWN_EXPLOSION_GRAVITY;
...
void Crown_Explosion_Interaction_Character(Elements *self, Elements *tar)
{
    ...
    Character_Hurt(tar, CROWN_EXPLOSION_DAMAGE);

    proj->durability --;
    proj->action_cooldown = CROWN_EXPLOSION_ACTION_COOLDOWN;
}
