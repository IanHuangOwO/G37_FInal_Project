...
#define WINNIE_EXPLOSION_IMG_PATH "assets/projectiles/maga_explosion.gif"
#define WINNIE_EXPLOSION_DURABILITY 1
#define WINNIE_EXPLOSION_ACTION_COOLDOWN 30
#define WINNIE_EXPLOSION_DAMAGE 50
#define WINNIE_EXPLOSION_BOUNCE_DECAY 0
#define WINNIE_EXPLOSION_COLLISION false
#define WINNIE_EXPLOSION_GRAVITY false

void Winnie_Explosion_Initialize(Projectile *proj) 
{
    proj->gif               = algif_new_gif(WINNIE_EXPLOSION_IMG_PATH, -1);
    proj->width             = proj->gif->width;
    proj->height            = proj->gif->height;
    proj->durability        = WINNIE_EXPLOSION_DURABILITY;
    proj->action_cooldown   = 0;
    proj->bounce_decay      = WINNIE_EXPLOSION_BOUNCE_DECAY;
    proj->collision         = WINNIE_EXPLOSION_COLLISION;
    proj->gravity           = WINNIE_EXPLOSION_GRAVITY;
...
void Winnie_Explosion_Interaction_Character(Elements *self, Elements *tar)
{
    ...
    Character_Hurt(tar, WINNIE_EXPLOSION_DAMAGE);

    proj->durability --;
    proj->action_cooldown = WINNIE_EXPLOSION_ACTION_COOLDOWN;
}
