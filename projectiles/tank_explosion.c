...
#define TANK_EXPLOSION_IMG_PATH "assets/projectiles/maga_explosion.gif"
#define TANK_EXPLOSION_DURABILITY 1
#define TANK_EXPLOSION_ACTION_COOLDOWN 30
#define TANK_EXPLOSION_DAMAGE 30
#define TANK_EXPLOSION_BOUNCE_DECAY 0
#define TANK_EXPLOSION_COLLISION false
#define TANK_EXPLOSION_GRAVITY false

void Tank_Explosion_Initialize(Projectile *proj) 
{
    proj->gif               = algif_new_gif(TANK_EXPLOSION_IMG_PATH, -1);
    proj->width             = proj->gif->width;
    proj->height            = proj->gif->height;
    proj->durability        = TANK_EXPLOSION_DURABILITY;
    proj->action_cooldown   = 0;
    proj->bounce_decay      = TANK_EXPLOSION_BOUNCE_DECAY;
    proj->collision         = TANK_EXPLOSION_COLLISION;
    proj->gravity           = TANK_EXPLOSION_GRAVITY;
...
void Tank_Explosion_Interaction_Character(Elements *self, Elements *tar)
{
    ...
    Character_Hurt(tar, TANK_EXPLOSION_DAMAGE);

    proj->durability --;
    proj->action_cooldown = TANK_EXPLOSION_ACTION_COOLDOWN;
}
