...
#define DEPORT_EXPLOSION_IMG_PATH "assets/projectiles/maga_explosion.gif"
#define DEPORT_EXPLOSION_DURABILITY 1
#define DEPORT_EXPLOSION_ACTION_COOLDOWN 30
#define DEPORT_EXPLOSION_DAMAGE 50
#define DEPORT_EXPLOSION_BOUNCE_DECAY 0
#define DEPORT_EXPLOSION_COLLISION false
#define DEPORT_EXPLOSION_GRAVITY false

void Deport_Explosion_Initialize(Projectile *proj) 
{
    proj->gif               = algif_new_gif(DEPORT_EXPLOSION_IMG_PATH, -1);
    proj->width             = proj->gif->width;
    proj->height            = proj->gif->height;
    proj->durability        = DEPORT_EXPLOSION_DURABILITY;
    proj->action_cooldown   = 0;
    proj->bounce_decay      = DEPORT_EXPLOSION_BOUNCE_DECAY;
    proj->collision         = DEPORT_EXPLOSION_COLLISION;
    proj->gravity           = DEPORT_EXPLOSION_GRAVITY;
...
void Deport_Explosion_Interaction_Character(Elements *self, Elements *tar)
{
    ...
    Character_Hurt(tar, DEPORT_EXPLOSION_DAMAGE);

    proj->durability --;
    proj->action_cooldown = DEPORT_EXPLOSION_ACTION_COOLDOWN;
}
