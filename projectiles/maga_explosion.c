...
#define MAGA_EXPLOSION_IMG_PATH "assets/projectiles/maga_explosion.gif"
#define MAGA_EXPLOSION_DURABILITY 1
#define MAGA_EXPLOSION_ACTION_COOLDOWN 30
#define MAGA_EXPLOSION_DAMAGE 30
#define MAGA_EXPLOSION_BOUNCE_DECAY 0
#define MAGA_EXPLOSION_COLLISION false
#define MAGA_EXPLOSION_GRAVITY false

void Maga_Explosion_Initialize(Projectile *proj) 
{
    proj->gif               = algif_new_gif(MAGA_EXPLOSION_IMG_PATH, -1);
    proj->width             = proj->gif->width;
    proj->height            = proj->gif->height;
    proj->durability        = MAGA_EXPLOSION_DURABILITY;
    proj->action_cooldown   = 0;
    proj->bounce_decay      = MAGA_EXPLOSION_BOUNCE_DECAY;
    proj->collision         = MAGA_EXPLOSION_COLLISION;
    proj->gravity           = MAGA_EXPLOSION_GRAVITY;
...
void Maga_Explosion_Interaction_Character(Elements *self, Elements *tar)
{
    ...
    Character_Hurt(tar, MAGA_EXPLOSION_DAMAGE);

    proj->durability --;
    proj->action_cooldown = MAGA_EXPLOSION_ACTION_COOLDOWN;
}
